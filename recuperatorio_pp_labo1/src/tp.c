...
int tp_ImprimirMenuSeisOpciones(char* descripcion,char* opcionUno, char* opcionDos, char* opcionTres, char* opcionCuatro, char* opcionCinco, char* opcionSeis)
{
    ...
    utn_GetNumeroInt(&opcionElegida, "\nIngresar opcion: ", "por favor, ingrese una opcion valida", 1, 9, 1);
...
int tp_ImprimirMenuOnceOpciones(char* descripcion,char* opcionUno, char* opcionDos, char* opcionTres, char* opcionCuatro, char* opcionCinco, char* opcionSeis,  char* opcionSiete, char* opcionOcho, char* opcionNueve, char* opcionDiez, char* opcionOnce)
{
    int opcionElegida;
    printf("%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s", descripcion, opcionUno, opcionDos, opcionTres, opcionCuatro, opcionCinco, opcionSeis, opcionSiete, opcionOcho, opcionNueve, opcionDiez, opcionOnce);
    utn_GetNumeroInt(&opcionElegida, "\n\nIngresar opcion: ", "por favor, ingrese una opcion valida", 1, 11, 1);
...
float tp_calcularPromedio(float acumulador, int total)
{
	float promedio;
	promedio = -1;
	//printf("DEBUG**** importe total: %.2f -- cantidad: %d", acumulador, total);
	if(total!= 0)
	{
	...
	{
		printf("error. El total debe ser distinto de 0");
	}
	return promedio;
}
