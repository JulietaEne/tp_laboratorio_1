...
/*######################## ______________-ARRAY-______________ ############################*/

/**
 * \brief 	Recibe un array de int y su extensión para imprimirloa modo de DEBUG
 * \param array[] Es el array recibido
 * \param len Es la longitud del array recibido
 * \return Debería retornar si fue exitoso 0, si no lo fue -1 SE PUEDE HACER ESTO???????? CÓMO SE PASARÍA? ??????
 *
 */
int array_imprimir(int array[], int len)
{
	int retorno=-1;
	int i;
	if(array != NULL && len > 0)
	{
		...
		for (i=0; i<len; i++)
		{
			printf("\n[DEBUG] Indice: %d - Valor: %d ",i, array[i]);
		}
	}
	return retorno;
}
...
/**
 * \brief 	Recibe un array de float y su extensión para imprimirloa modo de DEBUG
 * \param array[] Es el array recibido
 * \param len Es la longitud del array recibido
 * \return Debería retornar si fue exitoso 0, si no lo fue -1 SE PUEDE HACER ESTO???????? CÓMO SE PASARÍA? ??????
 *
 */
int arrayFloat_imprimir(float array[], int len)
{
	int retorno=-1;
	int i;
	if(array != NULL && len > 0)
	{
		...
		for (i=0; i<len; i++)
		{
			printf("\n[DEBUG] Indice: %d - Valor: %.2f ",i, array[i]);
		}
	}
	return retorno;
}
...
/**
 * \brief 	Ordena un array de enteros en forma DESCENDIENTE
 * \param array[] Es el array recibido
 * \param len Es la longitud del array recibido
 * \return Cantidad de iteraciones necesarias para ordenar si fue exitoso, si no lo fue -1
 *
 */
int arrayInt_ordenarDescendiente(int pArray[], int len)
{
	int retorno=-1;
	int i;
	int flagSwap;
	...
	if(pArray != NULL && len > 0)
	{
		...
		do{
			flagSwap=0;
			for (i=0; i<nuevoLimite; i++)//corto antes el límite, para estar seguro que siempre comparo contra un valor existente dentro del array
			{
				if(pArray[i] < pArray[i+1])
				{
					flagSwap = 1; //porque tengo que hacer un cambio de orden
					aux = pArray[i];
					pArray[i] = pArray[i+1];
					pArray[i+1] = aux;
				}
...
/**
 * \brief 	Recibe un array y su extensión para calcular el promedio de los elementos que lo componen
 * \param *pPromedio Puntero al espacio de memoria donde se dejará el resultado de la función
 * \param array[] Es el array recibido
 * \param len Es la longitud del array recibido
 * \return Retorna 0 si fue exitoso, si no lo fue -1
 */
int array_intPromediar(float* pPromedio, int array[], int len)
{
	int acumulador=0;
	int i;
	int retorno = -1;

	if (pPromedio != NULL && array != NULL)
	{
		for (i=0; i<len; i++)//leo el array
		{
			acumulador = acumulador + array[i];//sumo los elementos
		}
		*pPromedio = (float)acumulador / len; //tengo que castear porque las dos variables son int. - Paso el promedio x referencia
		retorno=0;
	}
 ...
 *
 */
int calcularMayorInt(int* pResultado, int numero1, int numero2)
{
	int retorno=-1;
	if(pResultado != NULL)
	{
...
/**
 * \brief 	Recibe un array y su extensión para identificar impares y borrarlos
 * \param array[] Es el array recibido
 * \param len Es la longitud del array recibido
 * \return Retorna 0 si fue exitoso, si no lo fue -1
 *
 */
int borrarImpares(int array[],int len)
{
	int retorno=-1;
	int i;
	if(array != NULL && len > 0)
