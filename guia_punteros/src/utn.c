...
/**
 * \brief Interactua con el usuario para solicitar un numero entero entre el 1 y el 100, con 1 posibilidad de error
 * \param int* numero Puntero al espacio de memoria donde se dejará el numero ingresado
 * \return Retorna 0 si el numero ingresado ha sido cargado exitosamente
 *		  		  -1 si el numero ingresado no ha podido ser cargado
 *
 */
int puntero_cargarInt(int* numero)
{
	int retorno;
	retorno = -1;
	if(numero != NULL)
	{
		if(!utn_GetNumeroInt(numero, "ingrese un valor: ", "Valor entre 1 y 100", 1, 100, 1))
		{
			retorno = 0;
		}
	}
