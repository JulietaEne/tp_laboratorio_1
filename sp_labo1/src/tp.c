...
float tp_calcularTotalConInteres(float numero, float interes)
{
	...
	if(interes <= 100 && interes >0)
	{
		...
		//printf("\nnumero: %.2f interes: %.2f - resultado %.2f*********\n\n",numero, interes, resultado);
	}
	return resultado;
}
...
int tp_ImprimirMenuDiezOpciones(char* descripcion,char* opcionUno, char* opcionDos, char* opcionTres, char* opcionCuatro, char* opcionCinco, char* opcionSeis,  char* opcionSiete, char* opcionOcho, char* opcionNueve, char* opcionDiez)
{
    int opcionElegida;
    printf("%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s", descripcion, opcionUno, opcionDos, opcionTres, opcionCuatro, opcionCinco, opcionSeis, opcionSiete, opcionOcho, opcionNueve, opcionDiez);
